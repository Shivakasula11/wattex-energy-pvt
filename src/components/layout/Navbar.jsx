...
import { NAV_LINKS, OFFERINGS, SOLUTIONS, EXPLORE_ITEMS } from '../../data/navLinks';
...
const Navbar = ({ forceSolid = false }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (offeringsDropdownRef.current && !offeringsDropdownRef.current.contains(event.target)) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        solid ? 'bg-white/95 backdrop-blur-xl shadow-xl py-3' : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 xl:px-12 flex justify-between items-center">
        {/* Logo */}
        <Link to="/" onClick={() => window.scrollTo(0, 0)} className="relative group flex items-center gap-3">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-solar-primary rounded-xl flex items-center justify-center transform group-hover:rotate-12 transition-all duration-300 shadow-lg">
            <span className="text-white text-xl md:text-2xl font-black font-heading leading-none">W</span>
          </div>
          <span
            className={`text-xl md:text-2xl font-black font-heading tracking-tighter transition-colors duration-300 ${
              solid ? 'text-solar-deep' : 'text-white'
            }`}
          >
            WATTEX<span className="text-solar-primary">ENERGY</span>
          </span>
        </Link>
        ...
        >
          ...
          <div
            ...
            onMouseEnter={() => { setIsOfferingsOpen(true); setIsSolutionsOpen(false); setIsExploreOpen(false); }}
          ...
          >
            ...
            >
              Solar Services
              <i className={`fas fa-chevron-down text-[8px] transition-transform duration-300 ${isOfferingsOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div
              className={`absolute top-full left-0 w-64 pt-2 transition-all duration-300 transform origin-top-left ${
                isOfferingsOpen ? 'scale-100 opacity-100 visible' : 'scale-95 opacity-0 invisible'
              }`}
            >
              <div className="bg-white rounded-2xl shadow-4xl border border-slate-100 overflow-hidden p-4 grid gap-1">
                {OFFERINGS.map((item) => (
                  ...
                  >
                    ...
                    <span className="block text-solar-deep text-[11px] font-black tracking-widest uppercase">{item.name}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div
            ...
            onMouseEnter={() => { setIsSolutionsOpen(true); setIsOfferingsOpen(false); setIsExploreOpen(false); }}
          ...
          >
            ...
            >
              Solar Solutions
              <i className={`fas fa-chevron-down text-[8px] transition-transform duration-300 ${isSolutionsOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div
              className={`absolute top-full left-0 w-64 pt-2 transition-all duration-300 transform origin-top-left ${
                isSolutionsOpen ? 'scale-100 opacity-100 visible' : 'scale-95 opacity-0 invisible'
              }`}
            >
              <div className="bg-white rounded-2xl shadow-4xl border border-slate-100 overflow-hidden p-4 grid gap-1">
                {SOLUTIONS.map((item) => (
          ...
          <div
            ...
            onMouseEnter={() => { setIsExploreOpen(true); setIsOfferingsOpen(false); setIsSolutionsOpen(false); }}
          ...
          >
            ...
            >
              Explore
              <i className={`fas fa-chevron-down text-[8px] transition-transform duration-300 ${isExploreOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div
              className={`absolute top-full right-0 w-64 pt-2 transition-all duration-300 transform origin-top-right ${
                isExploreOpen ? 'scale-100 opacity-100 visible' : 'scale-95 opacity-0 invisible'
              }`}
            >
              <div className="bg-white rounded-2xl shadow-4xl border border-slate-100 overflow-hidden p-4 grid gap-1">
      ...
      >
        <div className="flex flex-col p-8 gap-6 bg-slate-50/50 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            >
              Solar Services
              <i className={`fas fa-chevron-down text-sm transition-transform ${isOfferingsOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div className={`grid gap-4 pl-4 overflow-hidden transition-all duration-300 ${isOfferingsOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
              {OFFERINGS.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.path)}
          ...
          <div className="space-y-4">
            ...
            >
              Solar Solutions
              <i className={`fas fa-chevron-down text-sm transition-transform ${isSolutionsOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div className={`grid gap-4 pl-4 overflow-hidden transition-all duration-300 ${isSolutionsOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
              {SOLUTIONS.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.path)}
          ...
          <div className="space-y-4">
            ...
            >
              Explore
              <i className={`fas fa-chevron-down text-sm transition-transform ${isExploreOpen ? 'rotate-180' : ''}`}></i>
            </button>
            <div className={`grid gap-4 pl-4 overflow-hidden transition-all duration-300 ${isExploreOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
              {EXPLORE_ITEMS.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.path)}
