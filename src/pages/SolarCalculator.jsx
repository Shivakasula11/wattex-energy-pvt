...
export default function SolarCalculator() {
  ...
  const addCustomAppliance = () => {
    if (newCustomName.trim()) {
      setCustomAppliances([...customAppliances, { name: newCustomName, wattage: newCustomWattage }]);
      setNewCustomName('');
  ...
  const calculate = () => {
    const res = calculateSolarRequirements(applianceStates, customAppliances, backupHours);
    setResults(res);
  ...
  return (
    <div className="bg-slate-50 min-h-screen pt-24 pb-20">
      {/* Sub-Header */}
      <div className="bg-white border-b border-slate-100 sticky top-20 z-30 py-4 mb-12">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-solar-primary rounded-lg flex items-center justify-center text-white">
              <i className="fas fa-solar-panel text-sm"></i>
            </div>
            <span className="text-xl font-black text-solar-deep tracking-tight">Solar<span className="text-solar-primary">Calculator</span></span>
          </div>
        ...
        {activeTab === 'calculator' && (
          <>
            ...
            <div className="bg-white rounded-[2rem] shadow-xl p-8 lg:p-10 border border-slate-100 mb-10">
              ...
              <button onClick={addCustomAppliance} className="border-2 border-slate-100 text-solar-deep px-6 py-2.5 rounded-lg font-black uppercase tracking-widest text-[9px] hover:bg-slate-50 transition-all">+ Add Custom Appliance</button>
              {customAppliances.length > 0 && (
                <div className="mt-6 space-y-2">
                  {customAppliances.map((ca, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <span className="font-bold text-solar-deep text-sm">{ca.name}</span>
                      <span className="font-black text-solar-primary text-sm">{ca.wattage} watts</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Calculate Button */}
            <div className="mb-24">
              <button onClick={calculate} className="w-full bg-solar-primary hover:bg-rose-600 text-white py-4 rounded-lg font-black uppercase tracking-widest text-xs shadow-xl shadow-rose-200 transition-all flex items-center justify-center gap-3">
